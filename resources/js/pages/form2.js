...
export default function Form2(props) {
    ...
    function updateIsVehicle() {
        ...
        // document.getElementById("isVehicle-span").style.opacity = "0";
        document.getElementById("isVehicle-span").style.display = "none";
    }
    function updateSupplier() {
        document.getElementById('supplier').innerHTML = props.supplier
    ...
    function submitForm(e) {
        ...
        console.log(props.isVehicle)
        if (!props.isVehicle) {
            // document.getElementById("isVehicle-span").style.opacity = "1";
            document.getElementById("isVehicle-span").style.display = "block";
            validated = 0;
        }

        if (VALIDATE_ALPHA(props.isVehicle) == false) {
            document.getElementById("isVehicle-span").style.display = "block";
            validated = 0;
        }
        ...
        console.log(props.supplier)
        if (!props.supplier) {
            document.getElementById("supplier-span").style.display = "block";
            validated = 0;
        }

        if (VALIDATE_ALPHA(props.supplier) == false) {
            document.getElementById("supplier-span").style.display = "block";
            validated = 0;
        }
        if (validated == 1) {
    ...
    return (
        <ReactBootstrap.Row className="w99 h100">
                ...
                <ReactBootstrap.Col className=" " md={6}>
                    <svg className="float-right mt15 mr30" xmlns="http://www.w3.org/2000/svg" width="339" height="49" viewBox="0 0 339 49">
                        <g id="Group_4" data-name="Group 4" transform="translate(-512 -23)">
                            ...
                            <g id="Ellipse_7" data-name="Ellipse 7" transform="translate(802 23)" fill="none" stroke="#ccc" stroke-width="1">
                                <circle cx="24.5" cy="24.5" r="24.5" stroke="none" />
                                <circle cx="24.5" cy="24.5" r="24" fill="none" />
                            </g>
                            <line id="Line_12" data-name="Line 12" x2="95" transform="translate(561.5 47.5)" fill="none" stroke="#4d4acf" stroke-width="2" />
                            <line id="Line_13" data-name="Line 13" x2="95" transform="translate(706.5 47.5)" fill="none" stroke="#ccc" stroke-width="2" />
                            ...
                            <text id="_3" data-name="3" transform="translate(821 55)" fill="#ccc" font-size="20" font-family="AbrilFatface-Regular, Abril Fatface"><tspan x="0" y="0">3</tspan></text>
                        </g>
                    </svg>
                </ReactBootstrap.Col>
            ...
            <ReactBootstrap.Row className="d-flex align-items-center pl22">
                <form onSubmit={submitForm} enctype="multipart/form-data" method="post">
                    <ReactBootstrap.Row id="page2" className="mx-auto w75 form-container ">
                        <ReactBootstrap.Row>
                            ...
                            <ReactBootstrap.Col md={12}>
                                <label className="normal-text mt15" >Vehicle Make</label>

                                <select id="vehicleMake" onChange={e => { e.preventDefault(); props.setVehicleMake(e.target.value); }} className="select w50">
                                    <option>Vehicle Make 1</option>
                                    <option>Vehicle Make 2</option>
                                    <option>Vehicle Make 3</option>
                                    <option>Vehicle Make 4</option>
                                </select>
                            </ReactBootstrap.Col>
                            <ReactBootstrap.Col md={12}>
                                <label className="normal-text mt15">Vehicle Model</label>
                                <select id="vehicleModel" onChange={e => { e.preventDefault(); props.setVehicleModel(e.target.value); }} className="select w50">
                                    <option>Vehicle Model 1</option>
                                    <option>Vehicle Model 2</option>
                                    <option>Vehicle Model 3</option>
                                    <option>Vehicle Model 4</option>
                                </select>
                            </ReactBootstrap.Col>
                            <ReactBootstrap.Col md={12}>
                                <label className="normal-text mt15">Build Year</label>
                                <input id="buildYear" onChange={e => { e.preventDefault(); props.setBuildYear(e.target.value); }} className="w50" type="text" />
                            </ReactBootstrap.Col>
                        </ReactBootstrap.Row>
                        <ReactBootstrap.Row>
                            <ReactBootstrap.Col md={12}>
                                <label className="normal-text ">Is the vehicle..</label>
                            </ReactBootstrap.Col>
                            <ReactBootstrap.Col className="w19 mt10" md={2}>
                                <button onClick={e => { e.preventDefault(); props.setIsVehicle("New"); updateIsVehicle() }} className="w100 small-button-text">New</button>
                            </ReactBootstrap.Col>
                            <ReactBootstrap.Col className="w19 mt10" md={2}>
                                <button onClick={e => { e.preventDefault(); props.setIsVehicle("Used"); updateIsVehicle() }} className="w100 small-button-text">Used</button>
                                <br />
                            </ReactBootstrap.Col>
                            <ReactBootstrap.Col className="w19 mt10" md={2}>
                                <button onClick={e => { e.preventDefault(); props.setIsVehicle("Demo"); updateIsVehicle() }} className="w100 small-button-text">Demo</button>
                                <br />
                            </ReactBootstrap.Col>
                            <ReactBootstrap.Col className="w19 mt10" md={2}>
                                <button onClick={e => { e.preventDefault(); props.setIsVehicle("Unsure"); updateIsVehicle() }} className="w100 small-button-text">Unsure</button>
                                <br />
                            </ReactBootstrap.Col>
                            <input type="text" id="isVehicle" name="isVehicle" className="form-control" hidden />
                            <span id="isVehicle-span" className=" dn ml-2 p-2"><span className="color-red">* </span>Please select the vehicle type</span>
                        </ReactBootstrap.Row>

                        <ReactBootstrap.Row >
                            <ReactBootstrap.Col md={12}>
                                <label className="normal-text mt10">Who is the supplier?</label>
                            </ReactBootstrap.Col>
                            <ReactBootstrap.Col className="w19" md={2}>
                                <button onClick={e => { e.preventDefault(); props.setSupplier("Dealer"); updateSupplier() }} className="w100 mt10 small-button-text">Dealer</button>
                                <br />
                            </ReactBootstrap.Col>
                            <ReactBootstrap.Col className="w19" md={2}>
                                <button onClick={e => { e.preventDefault(); props.setSupplier("Refinance"); updateSupplier() }} className="w100 mt10 small-button-text">Refinance</button>
                                <br />
                            </ReactBootstrap.Col>
                            <ReactBootstrap.Col className="w19" md={2}>
                                <button onClick={e => { e.preventDefault(); props.setSupplier("Private Sale"); updateSupplier() }} className="w100 mt10 small-button-text">Private Sale</button>
                                <br />
                            </ReactBootstrap.Col>
                            <ReactBootstrap.Col className="w19" md={2}>
                                <button onClick={e => { e.preventDefault(); props.setSupplier("Auction"); updateSupplier() }} className="w100 mt10 small-button-text">Auction</button>
                                <br />
                            </ReactBootstrap.Col>
                            <ReactBootstrap.Col className="w19" md={2}>
                                <button onClick={e => { e.preventDefault(); props.setSupplier("Unsure"); updateSupplier() }} className="w100 mt10 small-button-text">Unsure</button>
                                <br />
                            </ReactBootstrap.Col>
                            <input type="text" id="supplier" name="supplier" className="form-control" hidden />
                            <span id="supplier-span" className=" dn ml-2 p-2"><span className="color-red">* </span>Please select the supplier</span>
                        </ReactBootstrap.Row>
                        <ReactBootstrap.Row>
                            <ReactBootstrap.Col className="mt15 mb20">
                                <button type="submit" className="w19 btn big-button-text bg-dark">Continue</button>
                            </ReactBootstrap.Col>
                        </ReactBootstrap.Row>
                    </ReactBootstrap.Row>
                </form>
